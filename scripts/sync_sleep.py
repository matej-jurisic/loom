#!/usr/bin/env python3
"""
Syncs Garmin sleep data into Loom as done Sleep occurrences.
...
LOOM_URL      = os.environ.get("LOOM_URL", "https://loom.operum.app")
LOOM_USERNAME = os.environ.get("LOOM_USERNAME", "")
LOOM_PASSWORD = os.environ.get("LOOM_PASSWORD", "")
...
def loom_login():
    r = requests.post(f"{LOOM_URL}/api/auth/login", json={
        "username": LOOM_USERNAME,
        "password": LOOM_PASSWORD,
    })
    r.raise_for_status()
...
def get_or_create_sleep_activity(token):
    ...
    r = requests.get(f"{LOOM_URL}/api/activities", headers=h)
    r.raise_for_status()
    for a in r.json():
        if a["title"] == "Sleep":
            print(f"  Found existing Sleep activity: {a['id']}")
            return a["id"]
    r = requests.post(f"{LOOM_URL}/api/activities", json={"title": "Sleep"}, headers=h)
    r.raise_for_status()
    aid = r.json()["id"]
...
def get_existing_sleep_starts(token, activity_id):
    h = {"Authorization": f"Bearer {token}"}
    r = requests.get(f"{LOOM_URL}/api/occurrences", params={"activityId": activity_id}, headers=h)
    r.raise_for_status()
    return [
...
def create_sleep(token, activity_id, start_dt, end_dt):
    ...
    r = requests.post(f"{LOOM_URL}/api/occurrences", json={
    ...
    occ_id = r.json()["id"]
    # Mark done
    r = requests.post(f"{LOOM_URL}/api/occurrences/{occ_id}/status",
...
def main():
    missing = [k for k in ("GARMIN_EMAIL", "GARMIN_PASSWORD", "LOOM_USERNAME", "LOOM_PASSWORD")
