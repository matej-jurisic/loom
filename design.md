...
- **View toggle, mobile (below `sm`):** three labelled segments don't fit next to the fold toggle and `+` on a phone header, so this is a single dial button instead: a small circular needle icon (pointing up/right/down for Day/3 days/Week, clockwise as the range grows) plus the current range's label as text, in the same `h-8 rounded-md border border-border` shell as the other header buttons. Tapping steps forward through the three ranges (wrapping from Week back to Day); the needle rotates to its new position with a 200ms transition. One tap target instead of three, and the label still makes the state unambiguous.
